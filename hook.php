...
function plugin_badges_install() {
   ...
   if (!$DB->tableExists("glpi_plugin_badges") && !$DB->tableExists("glpi_plugin_badges_badgetypes")) {
      $install = true;
      $DB->runFile(GLPI_ROOT . "/plugins/badges/sql/empty-2.2.2.sql");
   ...
   } else if($DB->tableExists("glpi_plugin_badges_badges") && !$DB->fieldExists("glpi_plugin_badges_badges", "is_recursive")){
      $DB->query("ALTER TABLE `glpi_plugin_badges_badges` ADD `is_recursive` tinyint(1) NOT NULL default '0';");
   }
