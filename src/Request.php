...
class Request extends CommonDBTM
{
    ...
    */
    ...
    public function showForUser($item, $options = [])
    {
        ...
        $initial = $this->listItems($item->fields['id'], ['begin_date' => $begin_date, 'end_date' => $end_date]);
    ...
    public function listItems($requesters_id, $options = [])
    {
        ...
        $message = TemplateRenderer::getInstance()->render('@badges/request_list.html.twig', [
            'rows' => $rows,
        ]);
     ...
     * Show badge request
     */
    public function showBadgeRequest()
    {
        ob_start();
        $this->loadAvailableBadges();
        $available_badges_html = ob_get_clean();

        ob_start();
        Html::showDateTimeField("affectation_date", ['value' => date('Y-m-d H:i:s')]);
        $affectation_date_field = ob_get_clean();
